...
/**
  * @brief  Initializes the W25Q16FV interface.
  * @retval None
  */
...
/**
  * @brief  Reads current status of the W25Q16FV.
  * @retval W25Q16FV memory status
  */
...
uint8_t BSP_W25Qx_Write(uint8_t* pData, uint32_t WriteAddr, uint32_t Size)
{
    ...
    {
        current_addr += W25Q16FV_PAGE_SIZE;
    }
    ...
    do
    {
        ...
        pData += current_size;
        current_size = ((current_addr + W25Q16FV_PAGE_SIZE) > end_addr) ? (end_addr - current_addr) : W25Q16FV_PAGE_SIZE;
...
uint8_t BSP_W25Qx_Erase_Block(uint32_t Address)
{
    ...
    {
        /* Check for the Timeout */
        if((HAL_GetTick() - tickstart) > W25Q16FV_SECTOR_ERASE_MAX_TIME)
        {
            return W25Qx_TIMEOUT;
        }
    }
...
uint8_t BSP_W25Qx_Erase_Chip(void)
{
    ...
    {
        /* Check for the Timeout */
        if((HAL_GetTick() - tickstart) > W25Q16FV_BULK_ERASE_MAX_TIME)
        {
            return W25Qx_TIMEOUT;
        }
    }
